 ...
 * Based on code by Simon Josefsson <simon@yubico.com> and Olov Danielson <olov@yubico.com>
 * Edited to be PEAR-less by Jason 'prototux' Fagnere <jason@prototux.net>
 *
 * Simple example:
 * <code>
...
class Auth_Yubico
{
	...
	/**
	 * Set the $_last_error and returns false
	 *
	 * @return false, just false.
	 * @access public
	 */
	function returnWithError($error_msg)
	{
	...
	function parsePasswordOTP($str, $delim = '[:]')
	{
		if (!preg_match("/^((.*)".$delim.")?"."(([cbdefghijklnrtuv]{0,16})"."([cbdefghijklnrtuv]{32}))$/i", $str, $matches))
			if (!preg_match("/^((.*)".$delim.")?"."(([jxe\.uidchtnbpygk]{0,16})"."([jxe\.uidchtnbpygk]{32}))$/i", $str, $matches)) // Dvorak?
				return false;
			else
	...
	function getParameters($parameters)
	{
		...
		foreach ($parameters as $param)
		{
			if(!preg_match("/".$param."=([0-9]+)/", $this->_response, $out))
				return $this->returnWithError('Could not parse parameter from response');
	...
	function verify($token, $use_timestamp = null, $wait_for_all = false, $sl = null, $timeout = null)
	{
		...
		if (!$ret)
			return $this->returnWithError('Could not parse Yubikey OTP');
		...
		while($URLpart = $this->getNextURLpart())
		{
			...
			curl_setopt($handle, CURLOPT_USERAGENT, "PEAR-less Auth_Yubico");
			...
			while ($info = curl_multi_info_read($mh))
			{
				if ($info['result'] == CURLE_OK)
				{
					...
					if (preg_match('/status=([a-zA-Z0-9_]+)/', $str, $out))
					{
						...
						/*
							There are 3 cases.
							1. OTP or Nonce values doesn't match - ignore response.
							2. We have a HMAC key.  If signature is invalid - ignore response.  Return if status = OK or status = REPLAYED_OTP.
					...
					if (!$wait_for_all && ($valid || $replay))
					{
						...
						curl_multi_close($mh);
						if ($replay)
							return $this->returnWithError('Replayed OTP');
						if ($valid)
							return true;
						return $this->returnWithError('Status: '.$status);
					}
		...
		*/
		...
		if ($valid)
			return true;
		return $this->returnWithError('No valid answer');
	}
}
?>
